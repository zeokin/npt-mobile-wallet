...
pub(crate) async fn scan_for_utxos(
    ...
    let block_heights_resp =
        rpc.block_heights_by_flags(flags.clone())
            .await
            .map_err(|e| match e {
                neptune_rpc_api::api::rpc::RpcError::Server(
                    neptune_rpc_api::model::json::JsonError::MethodNotFound,
    ...
    use neptune_wallet::triton_vm::prelude::Digest;
    ...
    for (idx, utxo_data) in discovered.iter_mut().enumerate() {
        ...
        let item = neptune_wallet::triton_vm::prelude::Tip5::hash(&utxo_data.utxo);
        ...
        let commitment = neptune_mutator_set::commit(item, sender_randomness, receiver_digest);
...
fn format_utxo_amount(utxo: &neptune_consensus::transaction::utxo::Utxo) -> String {
